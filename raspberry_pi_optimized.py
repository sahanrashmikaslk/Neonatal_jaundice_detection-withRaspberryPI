#!/usr/bin/env python3
"""
Optimized Neonatal Jaundice Detection for Raspberry Pi
This version implements multiple capture methods and robust error handling
"""
...
MAX_FAILED_FRAMES = 10  # Max consecutive failed frames before trying alternate method
CAMERA_RETRY_DELAY = 2  # Seconds to wait between camera reconnection attempts
...
def capture_frame_fswebcam(camera_device="/dev/video0", resolution="640x480"):
    ...
    try:
        ...
        log_message(f"fswebcam error: {str(e)}")
...
def create_capture_object(camera_index):
    """Create a capture object with optimized settings for Raspberry Pi"""
...
def main(camera_index=0, display=False, alert_threshold=0.7, continuous_mode=True):
    ...
    cap = create_capture_object(camera_index)
    
    # Track stats
    last_save_time = 0
    failed_frames_count = 0
    current_method = "opencv"  # Start with OpenCV method
    total_frames = 0
    successful_frames = 0
    ...
    try:
        while True:
            ...
            if current_method == "opencv":
                if not cap.isOpened():
                    log_message("Camera not open. Attempting to reopen...")
                    cap = create_capture_object(camera_index)
                ...
                if cap.isOpened():
                    ...
                    if not ret or frame is None or frame.size == 0:
                        ...
                        log_message(f"Failed to grab frame via OpenCV (attempt {failed_frames_count}/{MAX_FAILED_FRAMES})")
                        
                        # Switch method after too many failures
                        if failed_frames_count >= MAX_FAILED_FRAMES:
                            log_message("Switching to fswebcam method")
                            current_method = "fswebcam"
                            failed_frames_count = 0
                            if cap.isOpened():
                                cap.release()
                    else:
                        failed_frames_count = 0  # Reset counter on success
            
            # FSWEBCAM METHOD
            elif current_method == "fswebcam":
                ret, frame = capture_frame_fswebcam(camera_device)
                if not ret or frame is None or frame.size == 0:
                    failed_frames_count += 1
                    log_message(f"Failed to grab frame via fswebcam (attempt {failed_frames_count}/{MAX_FAILED_FRAMES})")
                    
                    # Switch back to OpenCV after too many failures
                    if failed_frames_count >= MAX_FAILED_FRAMES:
                        log_message("Switching back to OpenCV method")
            ...
            if frame is not None and frame.size > 0:
                ...
                # Log success rate occasionally
                if total_frames % 10 == 0:
                    success_rate = (successful_frames / total_frames) * 100
                    log_message(f"Camera capture success rate: {success_rate:.1f}% ({successful_frames}/{total_frames})")
